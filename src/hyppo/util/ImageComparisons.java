...
public class ImageComparisons {
	...
	public static double normalizedPSNR(DMatrixRMaj h1, DMatrixRMaj h2) {
		double mse = MSE(h1, h2);
		double[] minMax = EJMLExtensions.minMax(h1);
	...
	public static double SNR(DMatrixRMaj h1, DMatrixRMaj h2) {
		double var = EJMLExtensions.var(h1, null);
		double mse = MSE(h1, h2);
		return (double) (10 * Math.log10(var / mse));
	}
	...
	public static double powerSNR(DMatrixRMaj h1, DMatrixRMaj h2) {
		double pow = EJMLExtensions.power(h1);
		double mse = MSE(h1, h2);
		return (double) (10 * Math.log10(pow / mse));
	}
	...
	public static double MSR (DMatrixRMaj h1, DMatrixRMaj h2) {
		checkDimensions(h1, h2);
		double mean = EJMLExtensions.avg(h1);
		double std = (double) Math.sqrt(MSE(h1, h2));
		return mean / std;
	}
	
	private final static double k1 = 0.01f; //usually 0.01?
	private final static double k2 = 0.03f; //usually 0.03?
	...
	public static double SSIM (DMatrixRMaj h1, DMatrixRMaj h2, double dynRange) {
		...
		double mu1 = EJMLExtensions.avg(h1);
		double mu2 = EJMLExtensions.avg(h2);
		double v1 = EJMLExtensions.var(h1, mu1);
		double v2 = EJMLExtensions.var(h2, mu2);
		double sigma = EJMLExtensions.cov(h1, h2, mu1, mu2);
		double L = dynRange;
